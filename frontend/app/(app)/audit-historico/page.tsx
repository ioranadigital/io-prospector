    ...
    <div className="flex flex-col items-center gap-2">
      <div className={`relative ${box}`}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`${num} font-bold text-zinc-900 dark:text-white`}>{score}</span>
          {size === 'lg' && <span className="text-xs text-zinc-600 dark:text-zinc-500">/100</span>}
        </div>
      </div>
...
const scoreColor = (s: number) => (s >= 80 ? 'text-green-700 dark:text-green-400' : s >= 50 ? 'text-yellow-700 dark:text-yellow-400' : 'text-red-700 dark:text-red-400');
...
function TrendDelta({ logs }: { logs: AuditLog[] }) {
  ...
  const cls = delta > 0 ? 'text-green-700 dark:text-green-400' : delta < 0 ? 'text-red-700 dark:text-red-400' : 'text-zinc-600 dark:text-zinc-500';
...
export default function AuditHistoricoPage() {
  ...
  if (loading) return <div className="text-zinc-600 dark:text-zinc-500 text-center py-24">Cargando histórico...</div>;
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white flex items-center gap-2"><BarChart2 size={22} className="text-zinc-900 dark:text-white" /> Histórico de Auditorías</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Resultados guardados, agrupados por cliente</p>
        </div>
        <div className="flex gap-2">
          {(['all', '7days', '30days'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                filter === f ? 'bg-blue-600 text-zinc-900 dark:text-white' : 'bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white'
              }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-zinc-600 dark:text-zinc-500 mt-3">Score promedio global</p>
        ...
        <div className="lg:col-span-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 flex flex-col">
          <p className="text-xs text-zinc-600 dark:text-zinc-500 uppercase tracking-wider font-semibold mb-4">Checks acumulados</p>
          <div className="grid grid-cols-3 gap-3 flex-1">
            {[
              { label: 'Correctos', value: totalPass, icon: CheckCircle, color: 'text-green-700 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800/40' },
              { label: 'Avisos',    value: totalWarn, icon: AlertTriangle, color: 'text-yellow-700 dark:text-yellow-400', bg: 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800/40' },
              { label: 'Errores',   value: totalFail, icon: XCircle, color: 'text-red-700 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800/40' },
      ...
      <div>
        <h2 className="text-lg font-bold text-zinc-900 dark:text-white mb-3">Clientes auditados</h2>
        <div className="space-y-3">
            ...
            return (
              <div key={domain} className={`border rounded-xl overflow-hidden ${cardStyle(avg)}`}>
                <div
                  ...
                  className="w-full px-6 py-4 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 transition cursor-pointer"
                >
                  <div className="flex items-center gap-4">
                    <ScoreCircle score={avg} size="sm" />
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-bold text-base text-zinc-900 dark:text-white">{domain}</h3>
                        <TrendDelta logs={domainLogs} />
                      </div>
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{domainLogs.length} auditoría{domainLogs.length !== 1 ? 's' : ''}</p>
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-xs text-green-700 dark:text-green-400 flex items-center gap-0.5"><Check size={11} /> {pass}</span>
                        <span className="text-xs text-yellow-700 dark:text-yellow-400 flex items-center gap-0.5"><AlertTriangle size={11} /> {warn}</span>
                        <span className="text-xs text-red-700 dark:text-red-400 flex items-center gap-0.5"><X size={11} /> {fail}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-blue-600/20 border border-zinc-300 dark:border-zinc-700 hover:border-blue-600/50 text-zinc-700 dark:text-zinc-300 hover:text-blue-700 dark:hover:text-blue-300 rounded-lg text-xs font-medium transition"
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition" title="Eliminar cliente">
                      <Trash2 size={15} className="text-red-700 dark:text-red-400" />
                    </button>
                    {isExpanded ? <ChevronDown size={18} className="text-zinc-500 dark:text-zinc-400" /> : <ChevronRight size={18} className="text-zinc-500 dark:text-zinc-400" />}
                  </div>
                </div>
                    ...
                    {domainLogs
                      ...
                      .map(log => (
                        <div
                          ...
                          className="px-6 py-3 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 transition cursor-pointer group"
                        >
                          <div className="flex-1 min-w-0">
                            <p className="text-zinc-800 dark:text-zinc-200 font-mono text-xs truncate group-hover:text-blue-700 dark:group-hover:text-blue-300 transition flex items-center gap-1.5">
                              {log.url}
                              <ExternalLink size={11} className="text-zinc-400 dark:text-zinc-600 group-hover:text-blue-700 dark:group-hover:text-blue-400 flex-shrink-0" />
                            </p>
                            <p className="text-zinc-600 dark:text-zinc-500 text-xs mt-0.5">{fmtDate(log.created_at)}</p>
                          </div>
                          <div className="flex items-center gap-5 ml-4 flex-shrink-0">
                            ...
                            <div className="text-xs space-x-2">
                              <span className="text-green-700 dark:text-green-400 inline-flex items-center gap-0.5"><Check size={11} /> {log.pass_count}</span>
                              <span className="text-yellow-700 dark:text-yellow-400 inline-flex items-center gap-0.5"><AlertTriangle size={11} /> {log.warn_count}</span>
                              <span className="text-red-700 dark:text-red-400 inline-flex items-center gap-0.5"><X size={11} /> {log.fail_count}</span>
                            </div>
                            <span className="text-zinc-400 dark:text-zinc-600 text-xs">{log.duration_ms}ms</span>
                            <button
                              onClick={e => reAudit(log.url, e)}
                              disabled={reauditing === log.url}
                              className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded transition disabled:opacity-50" title="Volver a auditar esta URL">
                              <RotateCw size={12} className={`text-blue-700 dark:text-blue-400 ${reauditing === log.url ? 'animate-spin' : ''}`} />
                            </button>
                            <button
                              onClick={e => { e.stopPropagation(); setConfirmDel({ type: 'log', id: log.id, url: log.url }); }}
                              className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition" title="Eliminar análisis">
                              <Trash2 size={12} className="text-red-700 dark:text-red-400" />
                            </button>
                          </div>
                        </div>
                      ))}
          ...
          {grouped.length === 0 && (
            <p className="px-6 py-16 text-center text-zinc-600 dark:text-zinc-500 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl">
