  ...
  return (
    <svg width={width} height={height} className="overflow-visible">
      ...
      <line x1={pad} x2={width - pad} y1={pad + h - (50 / 100) * h} y2={pad + h - (50 / 100) * h}
        className="stroke-zinc-300 dark:stroke-zinc-700" strokeWidth="1" strokeDasharray="3 3" />
      <path d={line} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      {pts.map((p, i) => (
        <circle key={i} cx={p[0]} cy={p[1]} r={3} fill={strokeColor(values[i])} className="stroke-white dark:stroke-zinc-900" strokeWidth="1.5" />
      ))}
    </svg>
  );
}
...
function Delta({ value }: { value: number | null }) {
  if (value === null) return <span className="text-zinc-400 dark:text-zinc-600 text-xs">—</span>;
  const cls = value > 0 ? 'text-green-700 dark:text-green-400' : value < 0 ? 'text-red-700 dark:text-red-400' : 'text-zinc-600 dark:text-zinc-500';
...
function AuditClienteContent() {
  ...
  if (loading) return <div className="text-zinc-600 dark:text-zinc-500 text-center py-24">Cargando evolución...</div>;
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Evolución · {domain || '—'}</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-0.5">Comparativa de auditorías guardadas, por URL idéntica</p>
        </div>
      </div>

      {totalAudits === 0 ? (
        <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-6 py-16 text-center text-zinc-600 dark:text-zinc-500">
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
              <p className="text-zinc-600 dark:text-zinc-500 text-xs uppercase tracking-wider mb-2 flex items-center gap-1.5"><Clock size={13} /> Auditorías</p>
              <p className="text-3xl font-bold text-zinc-900 dark:text-white">{totalAudits}</p>
            </div>
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
              <p className="text-zinc-600 dark:text-zinc-500 text-xs uppercase tracking-wider mb-2 flex items-center gap-1.5"><Link2 size={13} /> URLs únicas</p>
              <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-400">{urlGroups.length}</p>
            </div>
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
              <p className="text-zinc-600 dark:text-zinc-500 text-xs uppercase tracking-wider mb-2 flex items-center gap-1.5"><TrendingUp size={13} /> Score más reciente</p>
              <p className={`text-3xl font-bold ${latest ? scoreColor(latest.total_score) : 'text-zinc-900 dark:text-white'}`}>{latest?.total_score ?? '—'}</p>
            </div>
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
              <p className="text-zinc-600 dark:text-zinc-500 text-xs uppercase tracking-wider mb-2">Última auditoría</p>
              <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 mt-2">{latest ? fmtDate(latest.created_at) : '—'}</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {urlGroups.map(([url, group]) => {
              ...
              return (
                <div key={url} className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                  {/* Cabecera de la URL */}
                  <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-4 flex-wrap">
                    <div className="min-w-0">
                      <a href={url} target="_blank" rel="noopener noreferrer"
                        className="text-sm font-mono text-blue-700 dark:text-blue-400 hover:underline flex items-center gap-1.5 truncate">
                        {url} <ExternalLink size={12} className="flex-shrink-0" />
                      </a>
                      <p className="text-xs text-zinc-600 dark:text-zinc-500 mt-0.5">{group.length} auditoría{group.length !== 1 ? 's' : ''} comparadas</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <Sparkline values={scores} />
                      <div className="text-right">
                        <p className="text-xs text-zinc-600 dark:text-zinc-500">primera → última</p>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-zinc-600 dark:text-zinc-500 text-xs uppercase tracking-wider border-b border-zinc-200 dark:border-zinc-800/60">
                    ...
                    <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                      ...
                      {[...group].reverse().map((log, idx, arr) => {
                        ...
                        return (
                          <tr key={log.id} className="hover:bg-zinc-100 dark:hover:bg-zinc-800/30 transition">
                            <td className="px-6 py-3 text-zinc-700 dark:text-zinc-300">{fmtDate(log.created_at)}</td>
                            ...
                            <td className="px-3 py-3 text-right text-xs space-x-2 whitespace-nowrap">
                              <span className="text-green-700 dark:text-green-400 inline-flex items-center gap-0.5"><Check size={12} />{log.pass_count}</span>
                              <span className="text-yellow-700 dark:text-yellow-400 inline-flex items-center gap-0.5"><AlertTriangle size={12} />{log.warn_count}</span>
                              <span className="text-red-700 dark:text-red-400 inline-flex items-center gap-0.5"><X size={12} />{log.fail_count}</span>
                            </td>
                            <td className="px-3 py-3 text-right text-zinc-400 dark:text-zinc-600 text-xs">{log.duration_ms}ms</td>
...
export default function AuditClientePage() {
  return (
    <Suspense fallback={<div className="text-zinc-600 dark:text-zinc-500 text-center py-24">Cargando...</div>}>
      <AuditClienteContent />
    </Suspense>
  );
}
